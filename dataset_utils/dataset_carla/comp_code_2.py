...
for scene in scenes:
    ...
    for image in images:
        ...
        info['cam_intrinsic'] = [[320.,   0., 800.],[  0., 320., 450.],[  0.,   0.,   1.]]
        info['width'] = 1600
        info['height'] = 900
    ...
    for gt in gts:
        ...
        if math.sqrt(float(gt_info[13])**2+float(gt_info[14])**2) < 30:
            ...
            if gt_info[2] == ' vehicle':
                ...
                info['category_id'] = 7
            info['iscrowd'] = 0
            info['bbox_cam3d'] = [float(gt_info[13]),float(gt_info[15]),float(gt_info[14]),float(gt_info[12]),float(gt_info[10]),float(gt_info[11]),-float(gt_info[16])]
            info['velo_cam3d'] = [0,0]
