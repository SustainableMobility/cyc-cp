...
class TwoStreamResCNNEncoder(nn.Module):
    """
    2-streams 2D CNN encoder using ResNet-152 pretrained where (1) image sequence stream and (2) optical flow sequence stream
    """

    def __init__(self, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300):
        """Load the pretrained ResNet-152 and replace top fc layer."""
        super(TwoStreamResCNNEncoder, self).__init__()

        self.fc_hidden1, self.fc_hidden2 = fc_hidden1, fc_hidden2
        self.drop_p = drop_p
        # image stream
        img_resnet = models.resnet152(pretrained=True)
        img_modules = list(img_resnet.children())[:-1]  # delete the last fc layer.
        self.img_resnet = nn.Sequential(*img_modules)
        self.img_fc1 = nn.Linear(img_resnet.fc.in_features, fc_hidden1)
        self.img_bn1 = nn.BatchNorm1d(fc_hidden1, momentum=0.01)
        self.img_fc2 = nn.Linear(fc_hidden1, fc_hidden2)
        self.img_bn2 = nn.BatchNorm1d(fc_hidden2, momentum=0.01)
        self.img_fc3 = nn.Linear(fc_hidden2, CNN_embed_dim)
        # optical flow stream
        ...
        # TODO: combine image and optical flow


    def forward(self, img_x_3d, opt_x_3d):

        # Image stream
        img_cnn_embed_seq = []
        for t in range(img_x_3d.size(1)):
            # ResNet CNN
            with torch.no_grad():
                img_x = self.img_resnet(img_x_3d[:, t, :, :, :])  # ResNet
                img_x = img_x.view(img_x.size(0), -1)  # flatten output of conv

            # FC layers
            img_x = self.img_bn1(self.img_fc1(img_x))
            img_x = F.relu(img_x)
            img_x = self.img_bn2(self.img_fc2(img_x))
            img_x = F.relu(img_x)
            img_x = F.dropout(img_x, p=self.drop_p, training=self.training)
            img_x = self.img_fc3(img_x)

            img_cnn_embed_seq.append(img_x)

        # swap time and sample dim such that (sample dim, time dim, CNN latent dim)
        img_cnn_embed_seq = torch.stack(img_cnn_embed_seq, dim=0).transpose_(0, 1)
        # cnn_embed_seq: shape=(batch, time_step, input_size)
        ...
        # combine embedding
        cnn_embed_seq = torch.cat([img_cnn_embed_seq, opt_cnn_embed_seq], dim=2)
        return cnn_embed_seq
...
class DecoderRNN(nn.Module):
    ...
    def __init__(self, CNN_embed_dim=300, h_RNN_layers=3, h_RNN=256, h_FC_dim=128, drop_p=0.3, num_classes=50):
        ...
        self.RNN_input_size = CNN_embed_dim*2
