...
class CNN3D(nn.Module):
    def __init__(self, t_dim=120, img_x=90, img_y=120, drop_p=0.2, fc_hidden1=256, fc_hidden2=128,
        ...
        if self.input_type == 'rgb_and_opt':
            self.opt_conv1 = nn.Conv3d(in_channels=1, out_channels=self.ch1, kernel_size=self.k1, stride=self.s1,
                                   padding=self.pd1)
            self.opt_bn1 = nn.BatchNorm3d(self.ch1)
            self.opt_conv2 = nn.Conv3d(in_channels=self.ch1, out_channels=self.ch2, kernel_size=self.k2, stride=self.s2,
                                   padding=self.pd2)
            self.opt_bn2 = nn.BatchNorm3d(self.ch2)
            self.opt_relu = nn.ReLU(inplace=True)
            self.opt_drop = nn.Dropout3d(self.drop_p)
            self.opt_pool = nn.MaxPool3d(2)
        ...
        if self.input_type == 'rgb_and_opt':
            fc_input_size = self.ch2 * self.conv2_outshape[0] * self.conv2_outshape[1] * self.conv2_outshape[2] * 2
        else:
            fc_input_size = self.ch2 * self.conv2_outshape[0] * self.conv2_outshape[1] * self.conv2_outshape[2]
        self.fc1 = nn.Linear(fc_input_size,
    ...
    def forward(self, x_3d):
        ...
        if self.input_type == 'rgb_and_opt':
            # Conv 1
            opt_x = self.opt_conv1(opt_x_3d)
            opt_x = self.opt_bn1(opt_x)
            opt_x = self.opt_relu(opt_x)
            opt_x = self.opt_drop(opt_x)
            # Conv 2
            opt_x = self.opt_conv2(opt_x)
            opt_x = self.opt_bn2(opt_x)
            opt_x = self.opt_relu(opt_x)
            opt_x = self.opt_drop(opt_x)
            # combine embedding
            x = torch.cat([x, opt_x], dim=2)
